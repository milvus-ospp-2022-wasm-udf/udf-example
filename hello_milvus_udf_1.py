...
###############################################################################
# -----------------------------------------------------------------------------
# hybrid search by udf greater_than
print(fmt.format("Start hybrid searching with `age64 > 1000` by UDF 'greater_than' [age64, 1000]"))
start_time = time.time()
wat_body = """(module
...
utility.create_function("greater_than", wat_body_base64, [DataType.INT64, DataType.INT64])
# result = hello_milvus_udf.search(vectors_to_search, "embeddings", search_params, limit=3, expr="UDF \"greater_than\" [age64, 1000]", output_fields=["random", "age64"])
# end_time = time.time()
